...
def my_data_view(request):
    ...
    """
    # --- 테스트용 임시 코드 ---
    # 실제 로그인 기능이 없으므로, 테스트용 유저를 DB에서 직접 가져옵니다.
    # 실제 배포 시에는 이 코드를 반드시 삭제해야 합니다.
    try:
    ...
    wishlist_qs = request.user.wished_courses.all()
    attending_qs = request.user.attending_courses.all()
